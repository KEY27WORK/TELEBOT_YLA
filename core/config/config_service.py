...
class ConfigService:
    """⚙️ Сервіс конфігурації для Telegram-бота YoungLA Ukraine.

    🔹 Призначення:
    - Завантажує конфігурацію з .env та config.json
    - Отримує курс валют із API НБУ (з кешем і fallback)
    - Керує локальними файлами ваги (`weights.json`) та типів товарів (`product_types.json`)
    - Працює як Singleton — один інстанс на процес

    📦 Можливості:
    - telegram_token / openai_api_key — доступ до токенів
    - fetch_exchange_rate() — надійне отримання курсу з кешем
    - load/save/update ваги та типів товарів
    - config.get() — отримання значення з config.json

    ✅ Надійна поведінка навіть без інтернету (fallback до базових курсів)
    """
