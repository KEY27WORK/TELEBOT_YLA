# 🧭 Модуль: WebDriverService (Playwright)

Модуль `web` відповідає за **стабільне та асинхронне завантаження HTML-сторінок** з сайту YoungLA з використанням Playwright.

Він використовується як **незалежний інфраструктурний компонент** усіма парсерами (`BaseParser`, `UniversalCollectionParser` тощо), щоб забезпечити:

- ізольовані вкладки для кожного запиту
- обхід Cloudflare через `stealth_async`
- автоматичні повторні спроби
- конфігурацію через `config.yaml`
...
- **Асинхронне керування ресурсами**: підтримує `async with`.
